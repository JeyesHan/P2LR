...
from examples.test import JEvaluator
...
def main_worker(args):
    ...
    for epoch in range(args.epochs):
        ...
        if ((epoch+1)%args.eval_step==0 or (epoch==args.epochs-1)):
            ...
            mAP = evaluator_ema.evaluate(test_loader_target, dataset_target.query, dataset_target.gallery, cmc_flag=False)
            is_best = mAP > best_mAP
            best_mAP = max(mAP, best_mAP)
            save_model(model_1_ema, (is_best and (1>0)), best_mAP, 1)
            save_model(model_2_ema, (is_best and (1<=0)), best_mAP, 2)
